...
{
    internal class NullableMappableEnumConverter<TEnum> : JsonConverter<Nullable<TEnum>>
        where TEnum : struct
    {
        private Dictionary<string, TEnum> _map;
        private Dictionary<TEnum, string> _reverseMap;

        public NullableMappableEnumConverter()
        {
        ...
        public override void Write(Utf8JsonWriter writer, TEnum? value, JsonSerializerOptions options)
        {
            ...
            if (_reverseMap.TryGetValue(value.Value, out string stringValue))
            {
